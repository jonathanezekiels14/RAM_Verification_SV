class ram_generator;
	...
	task run();
		for(int i=0;i<`num_of_transactions;i++) begin
			...
			gen_2_drv.put(gen_trans.copy());
			$display("[GENERATOR][%0t] Randomized  DATA_IN = %0H | ADDR = %0h | WRITE_ENB = %0b | READ_ENB = %0b",$time,gen_trans.data_in,gen_trans.address,gen_trans.write_enb,gen_trans.read_enb);
		end
	endtask
endclass
